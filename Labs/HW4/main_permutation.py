...
def internal_longest_path2(ribbon):
    result = ()
    flat_list = flatten(ribbon)
    perms_list = []
    permutation = list(flat_list)
    counter = 0
    while (set is not None):
        counter = counter + 1
        permutation = algL(permutation)
        perms_list.append(permutation)
    
    trails = make_trails(permutation, ribbon)
...
def longest_path(ribbon):
    ...
    try:
        ...
        result2 = internal_longest_path2(ribbon)
        if len(result) < 2:
            result = ()
