...
def run_test_file(inputs_filename, expected_filename):
    ...
    for index in range(0, len(inputs)):
        ...
        actual = main_withclass.heapsort(convert_to_int_list(input_list))
...
def test_args():
    result2 = main_withclass.heapsort(None)
    assert result2 == None
    print("None passes.")

    result5 = main_withclass.heapsort([])
    assert result5 == []
    print("empty list passes.")

    result5 = main_withclass.heapsort(["A", "B", "C"])
    assert result5 == None
