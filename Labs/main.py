...
def get_palindrome(pattern):
    ...
    for n in range(0, endIndex):
        ...
        compareResult = compare_elements(pattern, indexStart, indexEnd, indexOfElementRemoved)
...
def compare_elements(pattern, indexStart, indexEnd, indexOfElementRemoved):
    ...
    if (start == end):
        ...
        #its not a palindrome, but lets remove start and check startNext against end
        ...
        #its not a palindrome, but lets remove end and check endNext against start
...
# Your task is to write a routine: find_palindrome(pattern) which takes a tuple and returns a tuple or None.
    ...
    return result
# The returned tuple must be a palindrome which results from removing just one element from pattern. 
# So, for instance, if pattern was (3, 2, 1, 1, 2, 4, 3), the returned tuple would be (3, 2, 1, 1, 2, 3). 
# If more than one element must be removed from the tuple to make a palindrome, or a palindrome is not possible, 
